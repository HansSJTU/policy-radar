...
test('the September 4 content update retains MIT public-source verification', () => {
  const entry = getUpdateLog('zh').find(({ date }) => date === '2026-09-04');
...
test('the September 3 content update records the hearing, government opposition, and CPT evidence', () => {
  const entry = getUpdateLog('zh').find(({ date }) => date === '2026-09-03');
...
test('the September 2 content update records UC Berkeley thesis and dissertation CPT criteria', () => {
  const entry = getUpdateLog('zh').find(({ date }) => date === '2026-09-02');
...
test('the September 1 content update records the corrected hearing date and material CPT changes', () => {
  const entry = getUpdateLog('zh').find(({ date }) => date === '2026-09-01');

...
export const englishPolicyEditorial: PolicyEditorial[] = [
  {
    ...
    summary:
      'September 11 development (checked September 17): OIRA completed OPT fee proposal review with changes. The $100,000 figure remains media-reported. The official amount, payer, covered applications and exemptions are unpublished; student, employer or school payment cannot be assumed. It is not in effect.',
    background:
  ...
  },
{
  ...
  },
  {
    ...
    discovery: {
      ...
      next: { text: 'Public comments due November 10 before midnight ET' },
    },
    ...
    agency: 'DHS / USCIS',
    rin: '1615-AD22',
    status: 'NPRM published · Comments through November 10',
  ...
  },
  {
    ...
    discovery: {
      ...
      summary: 'DOL plans to rewrite PERM recruitment requirements and strengthen protections for U.S. workers.',
      audience: 'Employees and employers preparing PERM-based green-card sponsorship.',
    },
    ...
    effectState: 'not-in-effect',
    effectLabel: 'Not in effect · Submission has not changed current PERM rules',
    audience: 'Employees pursuing employer-sponsored green cards, and the employers filing PERM for them, including people currently in H-1B status.',
    caveat: 'This concerns permanent labor certification only. It is unrelated to H-1B cap selection and separate from the #06 wage-level proposal, RIN 1205-AC30.',
    summary: 'DOL plans to rewrite the PERM labor market test so that recruitment channels track how employers actually hire, while raising the bar for showing that no qualified U.S. worker could be found. The reform would also strengthen protections for laid-off U.S. workers and impose stricter nondiscrimination, hiring and recordkeeping duties on employers.',
    background: 'PERM is the labor certification an employer must obtain before sponsoring a foreign worker for an employment-based green card: the employer has to show it recruited U.S. workers in good faith and found no qualified person. The reform sits under 20 CFR 656, and DOL sent the proposed rule to the White House OIRA for prepublication review on September 14, 2026. The Unified Agenda originally targeted July 2026 for an NPRM; this September 17 check did not locate published text. A target month is not a publication date, and even less an effective date.',
    note: 'Submission does not change current rules; detailed terms and transition provisions await public text.',
    deck: 'Once an employer promises green-card sponsorship, what happens to the recruitment step?',
    headline: 'Preparing a green-card sponsorship case may take more time on recruitment and files.',
    ...
    possibilities: {
      ...
      items: [
        ['Drop the mandatory newspaper ad', 'Stop requiring a print advertisement in a local newspaper. Print ads are a holdover from the 2005 rule, and most technical job seekers never see them.'],
        ['Move to modern recruitment channels', 'Require or allow social media, job boards and other current channels, while dropping outdated ones such as radio and television.'],
        ['Stop requiring mailed resumes', 'Stop treating “paper applications by mail only” as compliant recruitment, so a physical step does not screen applicants out.'],
        ['Post roles on the company job page', 'If an employer has its own careers page, PERM roles should appear there, not only in a newspaper or on the state job board.'],
        ['Accept applications through the ATS', 'If an employer uses an applicant tracking system, applicants should be able to apply through it rather than to a single mailbox or mailing address.'],
      ],
    },
    teaser: 'If recruitment rules are rewritten, the work and timing of preparing a PERM case change with them.',
    impacts: [
      ['For employers: more recruitment and recordkeeping work', 'Higher minimum recruitment and recordkeeping standards could require process changes and additional evidence. Which new steps would be required, and what they would cost, cannot yet be calculated.'],
      ['For employers with layoffs: U.S.-worker protections could tighten', 'The agenda expressly calls for stronger protections for laid-off U.S. workers, but does not say how far back the lookback runs, which positions are covered, or whether each worker must be notified. It does not establish that a layoff bars a PERM filing.'],
      ['For H-1B employees: green-card timing could slip', 'If employer preparation takes longer, the employee’s green-card plan could shift. That is a conditional assessment; submission for review does not cancel existing H-1B status.'],
    ],
    ...
    scenarios: [
      ['Review continues, or the plan changes', 'Current rules keep applying', 'OIRA review remains open, or DOL revises the plan.', 'Employers can prepare cases under current rules. An agenda abstract alone cannot establish what requirements would be added later.', 'Whether OIRA closes review and whether DOL publishes a formal proposal.'],
      ['An NPRM opens for comment', 'Employers assess how the process would change', 'DOL publishes proposed terms and a comment window.', 'Employees and employers can size the preparation work against the text. Publication of a proposal does not make new requirements effective.', 'Recruitment standards, layoff protections, recordkeeping and proposed coverage.'],
      ['A final rule takes effect', 'PERM processing adjusts within the final scope', 'DOL completes rulemaking and the final rule reaches its effective date.', 'Whether new and existing cases must change depends on the final terms, transition provisions and any court orders.', 'The effective date, transition terms, and how recruitment already done and cases already filed are treated.'],
    ],
    scope: [
      ['Relevant process', 'PERM permanent labor certification under 20 CFR 656.'],
      ['H-1B connection', 'Mainly the later employer-sponsored green-card step, not a direct change to cap selection or H-1B eligibility.'],
      ['Current stage', 'Entered OIRA review September 14; this check did not locate a public NPRM, comment deadline or effective date.'],
    ],
  ...
  },
{
  ...
  "keyPoint": {
    "label": "Key point · The real change lands on visa and entry review",
    "text": "Of the stages listed in section 3(a), LCAs and petitions are paper filings an employer submits inside the United States, while **visa and entry review** happen face to face: a consular officer questions the applicant at the window and a CBP officer decides admission at the port, with the judgment made on the spot. By writing the sponsoring employer’s relevant layoffs into those two stages, the order brings company-level layoffs into the interview and the inspection. The people most likely to feel a difference are therefore those going for visa stamping or entering, not those already waiting on a domestic extension. The order does not say what an officer must ask, what an applicant must file beyond the existing record, or that a layoff bars a visa or admission; State and DHS implementation documents would address that. Until then, the preparation available is being able to explain how the role relates to the layoffs."
  },
  "impacts": [
    [
      "What counts as a “relevant layoff”: three conditions",
      "Section 3(a) directs that, in LCA, petition, **visa and entry review** involving H-1B workers entering or attempting to enter the United States, agencies consider direct or indirect sponsor layoffs within the previous year, or planned future layoffs, that negatively affect similarly situated U.S. workers. That breaks into three conditions. First, the employer must be the one filing the LCA or petition for this case; layoffs at a parent, affiliate or client matter only if they fall within the direct or indirect scope. Second, the roles cut must be comparable in duties and level to the offered job, so cuts in unrelated departments or at senior levels do not fall under this provision. Third, timing is limited to layoffs within the previous year and those already announced or planned, so older layoffs are outside it. These conditions decide whether a case falls inside the review scope; they do not decide the outcome, because the order sets no relationship between layoff counts or rates and the decision on a case. **Visa and entry review** are also where a consular officer and then a CBP officer decide on the spot, so the layoff question is asked there; LCAs and petitions are written filings by the employer and follow a different rhythm."
    ],
    [

...
const statsCopy = {
  zh: {
    ...
    first: '首条记录：',
    waiting: '等待第一条访问记录',
    countries: '最近 30 天访问国家',
    countryIntro: '按访问者所在国家汇总；国家统计从启用后开始累计。',
    countryVisitors: '日去重访客',
    countryViews: '浏览量',
    countryEmpty: '下一次主页访问后开始显示国家统计。',
    legend: '图例',
    method: '统计口径',
  ...
  en: {
    ...
    views: 'Page views',
    active: 'Days with traffic',
    daily: 'Daily traffic',
    first: 'First record: ',
    waiting: 'Waiting for the first visit',
    countries: 'Visitor countries over the last 30 days',
    countryIntro:
      'Grouped by visitor country; country data accumulates from the day this measurement is enabled.',
    countryVisitors: 'Daily unique visitors',
    countryViews: 'Page views',
    countryEmpty:
      'Country statistics will appear after the next home-page visit.',
    legend: 'Chart legend',
    method: 'Measurement method',
    visitorMethod: 'The same browser is counted once per calendar day.',
    viewMethod:
...
export default async function StatsPage({ searchParams }: StatsPageProps) {
  ...
  return (
    <main className="stats-page">
      ...
      <section className="stats-chart-card">
        <header>
          <div>
            <h2>{ui.daily}</h2>
            <p>
              {firstActiveDay ? `${ui.first}${firstActiveDay}` : ui.waiting}
            </p>
          </div>
      ...
      <section className="stats-country-card">
        <header>
        ...
        {countries.length > 0 ? (
          <ol className="country-ranking">
            {countries.map((country) => (
              <li key={country.country}>
                <span className="country-code">{country.country}</span>
                <div className="country-detail">
                  <div>
                    <strong>
                      {getCountryLabel(country.country, language)}
                    </strong>
                    <span>
                      {country.visitors.toLocaleString(ui.locale)}{' '}
                      {ui.countryVisitors} ·{' '}
                      {country.pageViews.toLocaleString(ui.locale)}{' '}
                      {ui.countryViews}
                    </span>
                  </div>
                  <span className="country-bar" aria-hidden="true">
                    <span
                      style={{
                        width: `${Math.max(4, (country.visitors / maxCountryVisitors) * 100)}%`,
                      }}
                    />
                  </span>
                </div>
          ...
          <p className="country-empty">{ui.countryEmpty}</p>
        )}
      </section>
      ...
      <section className="stats-method">
        ...
        <div>
          <p>
            <strong>{ui.visitors}: </strong>
            {ui.visitorMethod}
          </p>
          <p>
            <strong>{ui.views}: </strong>
            {ui.viewMethod}
          </p>
          <p>
            <strong>{language === 'en' ? 'Privacy' : '隐私'}: </strong>
            {ui.privacy}
          </p>
        </div>
      </section>
